...
typedef struct UI {
    ...
    void render_bar(const Project &project, size_t page, size_t total) {
        ...
        // 当前绘制的位置
        size_t x = 0; // currentX
        render_text(&x, 0, ColorText("uPET Editor:", "\x1b[30;47m"));
        x += 1;
        render_text(&x, 0,
                    ColorText(project.project_name.empty()
                                  ? "None"
     ...
     */
    void render_note(const Note &note, size_t count, size_t y, size_t column, double tempo,
                     bool show_sec, bool show_act, bool highlight) {
        // pos + page * (size().y - 3)
        ...
        render_text(
            &x, y,
            ColorText(temp, lyric_base + (highlight ? "\x1b[30;47m" : "")));
        ...
        render_text(
            &x, y,
            ColorText(
                note.Lyric.length() >= 16 ? (note.Lyric.substr(0, 13) + "...")
                                          : note.Lyric,
        ...
        render_text(
            &x, y,
            ColorText(
                note.Flags.length() >= 16 ? (note.Flags.substr(0, 13) + "...")
                                          : note.Flags,
...
typedef struct Editor {
    ...
    Editor()
        : count(0), column(0), _show_sec(true), _show_actual_notenum(true) {}
